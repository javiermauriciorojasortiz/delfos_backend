...
class Notificador extends APPBASE {
    ...
    function consultarNotificadores() {
      $rta = $this->obtenerResultset(QUERY_SEG::_NTF_CONSULTAR, $this->listarParamRequeridos());
      return $rta;
    ...
    public function establecerNotificador(int $id, bool $nuevo){
      ...
      if($nuevo) {//Insertar
        ...
        $this->insertarAuditoria(self::$usuarioID,ENUM_AUD::NOTIFICADOR, $observacion, true, "I", ""); //Existe el usuario
      } else { //Actualizar
        
        $rta = $this->actualizarData(QUERY_SEG::_NTF_ACTUALIZAR, $params, true);
        $observacion = "Usuario ID: " . $params["id"] . ". Identificacion: " . $this->parametros["identificacion"];
        $this->insertarAuditoria(self::$usuarioID,ENUM_AUD::NOTIFICADOR, $observacion, true, "M", ""); //Existe el usuario  
      }
      return $rta;
    }
